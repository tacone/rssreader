		...
		{:else}
			<ul class="flex flex-col">
				{#each data.items as item (item.id)}
					<li>
						<a href="/dashboard/r/{data.feedSlug}/{item.slug}" class="block border-b border-base-200 px-4 py-3 hover:bg-base-200 {($page.url.pathname + $page.url.search).includes(item.slug) ? 'bg-base-200' : ''}">
